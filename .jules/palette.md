...
## 2026-04-27 - Add search landmark roles
**Learning:** Search input container elements (e.g., wrappers with classes like `.search-controls` or `.shop-controls`) should use the `role="search"` ARIA landmark to enable quick navigation for screen reader users.
**Action:** Always add `role="search"` to the container element wrapping search inputs.
