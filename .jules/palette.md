...
## 2026-06-05 - Add Native Enter Form Submission on Teacher portal inputs and ARIA labels on ad-hoc modals
**Learning:** Text inputs within standalone portal configurations (like the Teacher settings `pinInput` and `durationInput`) lack native \`Enter\` key form submission because they aren't wrapped in standard \`<form>\` tags. Also, ad-hoc inputs in custom modals (like `pinVerifyInput`) often miss \`aria-label\`s or visible labels. This breaks keyboard accessibility because users cannot submit forms efficiently using 'Enter', and assistive technologies cannot properly announce the input field.
**Action:** Always verify that ad-hoc configuration inputs and modal fields are equipped with a \`keydown\` listener simulating the 'Enter' key submission by clicking the primary action button. Ensure all standalone inputs have an explicit \`aria-label\` or \`<label>\` attribute.
