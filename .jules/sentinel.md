...
## 2025-03-01 - CSP Security Enhancement\n**Vulnerability:** Missing Content Security Policy (CSP) headers across all responses.\n**Learning:** vercel.json headers routing rule was not applying CSP alongside other security headers like X-Content-Type-Options.\n**Prevention:** Apply a strict CSP baseline to the global `/(.*)` header block to enforce security in depth.
