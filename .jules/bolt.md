...
## 2025-05-18 - Avoid redundant object normalization on in-memory cache searches
**Learning:** Functions that search an entire memory cache (like `findStripeBillingProfiles` traversing `memoryState.values`) can cause massive CPU spikes if they run expensive object deep-cloning or normalization routines (like `normalizeBillingProfile`) *before* checking if the entry matches the search criteria.
**Action:** When searching a cached memory store map or array, perform the fast-path lookup assertions (e.g. `normalizeEmail(raw.customerEmail) === normalizedCustomerEmail`) against the raw object first, and only execute the expensive extraction/normalization logic on the matching result.
