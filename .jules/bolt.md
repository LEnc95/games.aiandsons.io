...
## 2024-06-10 - Optimize in-memory collection queries to reduce normalization allocations
**Learning:** In `api/stripe/_family-store.js`, chained map/filter pipelines on in-memory collections resulted in O(N) intermediate array allocations and called expensive normalization functions on every object before filtering.
**Action:** Replace map/filter chains with imperative `for...of` loops that filter against raw properties first and only normalize matching items to avoid redundant allocations.
