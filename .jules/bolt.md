...
## 2024-05-18 - Avoid array allocation and redundant object normalizations in iterations over Maps
**Learning:** Chaining `.values()`, `.map()`, and `.filter()` over in-memory `Map` collections creates an intermediate array containing fully normalized objects for *every* entry in the Map, regardless of whether they pass the final `.filter()` condition. In environments without pagination (like testing or local memory fallback), this causes O(N) memory allocation and wastes CPU cycles on normalizations that get discarded immediately.
**Action:** When filtering in-memory `Map` or `Set` states, use a single `for...of` loop over `collection.values()`. Apply early-return filter conditions against raw properties first, and only call normalization functions on matching records, using `.push()` to populate the final array. This strategy replaces O(N) normalizations and intermediate allocations with a single pass yielding only the necessary O(K) normalizations.
