...
function drawAvatar(p,x,y,r=22){ctx.fillStyle=p?.color||"#31e6c1";ctx.beginPath();ctx.arc(x,y,r,0,Math.PI*2);ctx.fill();ctx.fillStyle="#fff";ctx.textAlign="center";ctx.textBaseline="middle";ctx.font=`${Math.round(r*1.25)}px "Segoe UI Emoji",sans-serif`;ctx.fillText(p?.avatar||"🦊",x,y+1);}
...
function drawRevealPlayers(snapshot){const left=(snapshot.players||[]).filter((p)=>p.choice==="left"),right=(snapshot.players||[]).filter((p)=>p.choice==="right");[[left,315],[right,885]].forEach(([list,cx])=>list.forEach((p,i)=>{const spacing=snapshot.duel&&list.length===2?160:76,x=cx+(i-(list.length-1)/2)*spacing,y=snapshot.duel?540:555;drawAvatar(p,x,y,21);ctx.strokeStyle="#fff";ctx.lineWidth=p.roundPoints?4:1;ctx.stroke();if(snapshot.duel){centerText(p.name,x,y+31,145,13,"#fff");centerText(`Read ${String(p.prediction||"—").toUpperCase()} ${p.readCorrect?"✓":"×"}`,x,y+49,145,12,p.readCorrect?"#58edcb":"#ffb0c2");centerText(`+${(p.roundPoints||0).toLocaleString()}${p.hotTake?" 🔥":""}`,x,y+68,145,13,"#ffe36e");}else{centerText(p.name,x,y+31,72,12,"#fff");if(p.roundPoints)centerText(`+${p.roundPoints}`,x,y+50,74,12,"#ffe36e");}}));centerText(snapshot.resultHeadline||"The crowd has spoken!",600,650,1050,26,"#ffe36e");}
function drawScoreboard(snapshot,title="SCOREBOARD"){centerText(title,600,90,1000,66,"#ffe36e");const players=snapshot.players||[];players.slice(0,8).forEach((p,i)=>{const col=i%2,row=Math.floor(i/2),x=145+col*500,y=160+row*95;rounded(x,y,410,70,18,"rgba(255,255,255,.07)",i===0?"#ffe36e":"rgba(255,255,255,.1)");drawAvatar(p,x+38,y+35,18);ctx.textAlign="left";ctx.fillStyle="#fff";ctx.font="850 23px Inter,system-ui";ctx.fillText(`${i+1}. ${p.name}`,x+72,y+31);ctx.fillStyle="#b9acd0";ctx.font="700 14px Inter,system-ui";ctx.fillText(p.roundPoints?`+${p.roundPoints.toLocaleString()} this round`:"No points this round",x+72,y+53);ctx.textAlign="right";ctx.fillStyle="#ffe36e";ctx.font="950 24px Inter,system-ui";ctx.fillText((p.points||0).toLocaleString(),x+385,y+43);});}
function drawPodium(snapshot){centerText(snapshot.duel?"DUEL SHIFT CHAMPION":"FINAL CROWD",600,80,1000,62,"#ffe36e");const top=(snapshot.players||[]).filter((p)=>p.rank>0).slice(0,3);const spots=[{p:top[0],x:600,y:205,h:300},{p:top[1],x:330,y:275,h:230},{p:top[2],x:870,y:320,h:185}];spots.forEach((spot,index)=>{const p=spot.p;if(!p)return;drawAvatar(p,spot.x,spot.y-35,38);rounded(spot.x-105,spot.y,210,spot.h,22,index===0?"#9c7533":"rgba(255,255,255,.1)",index===0?"#ffe36e":"rgba(255,255,255,.2)");centerText(p.name,spot.x,spot.y+48,180,25,"#fff");centerText(String(p.rank),spot.x,spot.y+105,100,64,"#ffe36e");centerText((p.points||0).toLocaleString(),spot.x,spot.y+spot.h-30,160,22,"#fff");});centerText(snapshot.duel?"Seven rounds of reads, risks, and rivalry":`${snapshot.unanimousRounds||0} unanimous round${snapshot.unanimousRounds===1?"":"s"}`,600,640,800,24,"#b9acd0");}
