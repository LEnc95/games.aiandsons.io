  ...
  try {
    ...
    const initialState = await page.evaluate(() => {
      ...
      return {
        ...
        todayDailyHref: document.querySelector('#todayPanel a[data-today-kind="daily"]')?.getAttribute('href') || '',
    ...
    assert(initialState.todayDailyHref.startsWith('/'), `Expected the Today daily mission to link to its game: ${JSON.stringify(initialState)}`);
