...
async function waitFor(url, timeoutMs = 30000) { const deadline=Date.now()+timeoutMs; while(Date.now()<deadline){try{if((await fetch(url)).ok)return;}catch{} await new Promise((resolve)=>setTimeout(resolve,250));} throw new Error(`Timed out waiting for ${url}`); }
function stop(proc){if(!proc||proc.killed)return;if(process.platform==="win32"&&proc.pid)spawnSync("taskkill",["/pid",String(proc.pid),"/t","/f"],{stdio:"ignore"});else proc.kill("SIGTERM");}
async function main(){
